...
export const P5_CALL_BLOCKS = [
  ...
  { type: 'p5_point', call: 'p5.point', kind: 'statement', colour: '#E6E8E2', tooltip: '点をうちます',
    message: '点 x %1 y %2',
    args: [{ name: 'X', shadow: 200 }, { name: 'Y', shadow: 200 }] },
  { type: 'p5_arc', call: 'p5.arc', kind: 'statement', colour: '#E6E8E2', tooltip: '弧をかきます（角度はラジアン）',
  ...
  // --- いろ ---
  { type: 'p5_background', call: 'p5.background', kind: 'statement', colour: '#EBEDE7', tooltip: '背景の色を決めます',
    message: '背景色 赤 %1 緑 %2 青 %3',
    args: [{ name: 'R', shadow: 240 }, { name: 'G', shadow: 240 }, { name: 'B', shadow: 250 }] },
  { type: 'p5_fill', call: 'p5.fill', kind: 'statement', colour: '#EBEDE7', tooltip: '塗りつぶしの色を決めます',
    message: '塗り色 赤 %1 緑 %2 青 %3',
    args: [{ name: 'R', shadow: 255 }, { name: 'G', shadow: 100 }, { name: 'B', shadow: 100 }] },
  { type: 'p5_no_fill', call: 'p5.no_fill', kind: 'statement', colour: '#EBEDE7', tooltip: '塗りつぶしをやめます',
    message: '塗りつぶしなし', args: [] },
  { type: 'p5_stroke', call: 'p5.stroke', kind: 'statement', colour: '#EBEDE7', tooltip: '輪郭の色を決めます',
    message: '線の色 赤 %1 緑 %2 青 %3',
    args: [{ name: 'R', shadow: 60 }, { name: 'G', shadow: 60 }, { name: 'B', shadow: 90 }] },
  { type: 'p5_no_stroke', call: 'p5.no_stroke', kind: 'statement', colour: '#EBEDE7', tooltip: '輪郭をやめます',
    message: '輪郭なし', args: [] },
  { type: 'p5_stroke_weight', call: 'p5.stroke_weight', kind: 'statement', colour: '#EBEDE7', tooltip: '線の太さを決めます',
    message: '線の太さ %1', args: [{ name: 'W', shadow: 3 }] },

  // --- もじ ---
  { type: 'p5_text', call: 'p5.text', kind: 'statement', colour: '#F0F1EC', tooltip: '文字をかきます',
    message: '文字 %1 を x %2 y %3 に',
    args: [{ name: 'T', text: 'Hello' }, { name: 'X', shadow: 100 }, { name: 'Y', shadow: 100 }] },
  { type: 'p5_text_size', call: 'p5.text_size', kind: 'statement', colour: '#F0F1EC', tooltip: '文字の大きさを決めます',
    message: '文字の大きさ %1', args: [{ name: 'S', shadow: 24 }] },

  // --- へんかん ---
  { type: 'p5_push', call: 'p5.push', kind: 'statement', colour: '#E1E3DD', tooltip: '今の状態を保存します',
    message: '状態を保存', args: [] },
  { type: 'p5_pop', call: 'p5.pop', kind: 'statement', colour: '#E1E3DD', tooltip: '保存した状態に戻します',
    message: '状態を戻す', args: [] },
  { type: 'p5_translate', call: 'p5.translate', kind: 'statement', colour: '#E1E3DD', tooltip: '原点を動かします',
    message: '原点を動かす x %1 y %2',
    args: [{ name: 'X', shadow: 200 }, { name: 'Y', shadow: 200 }] },
  { type: 'p5_rotate', call: 'p5.rotate', kind: 'statement', colour: '#E1E3DD', tooltip: '回転します（ラジアン）',
    message: '回転する %1', args: [{ name: 'A', shadow: 0.785 }] },
  { type: 'p5_scale', call: 'p5.scale', kind: 'statement', colour: '#E1E3DD', tooltip: '拡大・縮小します',
    message: '拡大 横 %1 倍 縦 %2 倍',
    args: [{ name: 'X', shadow: 2 }, { name: 'Y', shadow: 2 }] },
  { type: 'p5_clear', call: 'p5.clear', kind: 'statement', colour: '#E1E3DD', tooltip: 'キャンバスを消します',
    message: 'キャンバスを消す', args: [] },
  { type: 'p5_reset_matrix', call: 'p5.reset_matrix', kind: 'statement', colour: '#E1E3DD',
    tooltip: '移動・回転・拡大をすべて元にもどします',
    message: '座標を元にもどす', args: [] },

  // --- 値として使うもの ---
  { type: 'p5_random', call: 'random', kind: 'value', output: 'Number', colour: '#DCDED8',
    tooltip: '最小値から最大値までのランダムな数',
    message: 'ランダムな数 %1 〜 %2',
    args: [{ name: 'A', shadow: 0 }, { name: 'B', shadow: 400 }] },
  { type: 'p5_map', call: 'map_value', kind: 'value', output: 'Number', colour: '#DCDED8',
    tooltip: '値の範囲を変換します',
    message: '%1 を %2 〜 %3 から %4 〜 %5 に変換',
    args: [{ name: 'V', shadow: 100 }, { name: 'A1', shadow: 0 }, { name: 'A2', shadow: 400 },
           { name: 'B1', shadow: 0 }, { name: 'B2', shadow: 100 }] },
  { type: 'p5_cos', call: 'cos', kind: 'value', output: 'Number', colour: '#DCDED8',
    tooltip: 'コサイン（ラジアン）', message: 'cos %1', args: [{ name: 'A', shadow: 0 }] },
  { type: 'p5_sin', call: 'sin', kind: 'value', output: 'Number', colour: '#DCDED8',
    tooltip: 'サイン（ラジアン）', message: 'sin %1', args: [{ name: 'A', shadow: 0 }] },
];

/** 名前だけの値ブロック（変数のように使う） */
export const P5_NAME_BLOCKS = [
  { type: 'p5_frame_count', name: 'frameCount', label: 'フレーム数', colour: '#DCDED8', output: 'Number',
    tooltip: 'アニメーションが始まってからのコマ数' },
  { type: 'p5_width', name: 'p5.width', label: 'キャンバスの幅', colour: '#DCDED8', output: 'Number',
    tooltip: 'キャンバスの横の大きさ' },
  { type: 'p5_height', name: 'p5.height', label: 'キャンバスの高さ', colour: '#DCDED8', output: 'Number',
    tooltip: 'キャンバスの縦の大きさ' },
  { type: 'p5_pi', name: 'PI', label: '円周率 π', colour: '#DCDED8', output: 'Number',
    tooltip: '約 3.14159' },
];

/** def setup(): / def draw(): をそのままブロックにしたもの */
export const DEF_BLOCKS = [
  { type: 'p5_setup', name: 'setup', message: '最初に1回だけ %1', colour: '#D3D6CF',
    tooltip: 'ページを開いたときに1回だけ実行されます' },
  { type: 'p5_draw', name: 'draw', message: 'くり返し描く %1', colour: '#D3D6CF',
  ...
  Blockly.defineBlocksWithJsonArray([
    {
      ...
      output: 'String', colour: '#F0F1EC', inputsInline: true,
      tooltip: 'input() でキーボードから文字列を受け取ります',
    },
    {
      type: 'py_to_int', message0: '整数にする %1',
      args0: [{ type: 'input_value', name: 'VALUE' }],
      output: 'Number', colour: '#DCDED8', inputsInline: true, tooltip: 'int() で整数に変換します',
    },
    {
      type: 'py_to_float', message0: '小数にする %1',
      args0: [{ type: 'input_value', name: 'VALUE' }],
      output: 'Number', colour: '#DCDED8', inputsInline: true, tooltip: 'float() で小数に変換します',
    },
    {
      type: 'py_to_text', message0: '文字列にする %1',
      args0: [{ type: 'input_value', name: 'VALUE' }],
      output: 'String', colour: '#F0F1EC', inputsInline: true, tooltip: 'str() で文字列に変換します',
    },
    {
      type: 'py_comment', message0: 'メモ %1',
      args0: [{ type: 'field_input', name: 'TEXT', text: 'ここに説明' }],
      previousStatement: null, nextStatement: null, colour: '#D3D6CF',
    ...
    },
    {
      ...
      output: null, colour: '#D3D6CF', tooltip: 'Python の式をそのまま書きます',
    },
  ]);
